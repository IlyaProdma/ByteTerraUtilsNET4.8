...
namespace ByteTerraUtils
{
    public class GoogleUtils
    {
        ...
        public int GetIndexBySessionNumber(string spreadSheetId, string sheet, int sessionNumber)
        {
        ...
        public void SetTrackDetectorsInfo(string spreadSheetId, int sessionNumber, List<double> trackDetectorsInfo)
        {
            ...
            for (int i = trackDetectorsInfo.Count - 1; i > 0; --i)
            {
        ...
        public void SetNewSessionInfo(string spreadSheetId, Session session)
        {
            ...
            dataValues.Add(session.IrradiationAngle == -1 ? "-" : session.IrradiationAngle.ToString());
            dataValues.Add(session.Pressure == -1 ? "-" : session.Pressure.ToString());
            dataValues.Add(session.Humidity == -1 ? "-" : session.Humidity.ToString());
            dataValues.Add(session.TemperatureTest == -1 ? "-" : session.TemperatureTest.ToString());
            dataValues.Add(session.TemperatureSession == -1 ? "-" : session.TemperatureSession.ToString());
